...
inline void iallocator_join(_interface iallocator) {
...
inline void* iallocator_acquire_v(_interface iallocator, int size, const char* file, int line) {
	object oheap = __object_from_interface(iallocator);
	dbg_assert(__cast(oheap, IALLOCATOR_ID) == iallocator);

	return ((struct iallocator_vtable*)iallocator->__vtable)->__acquire(oheap, size, file, line);
}
inline bool iallocator_release_v(_interface iallocator, void* buff, const char* file, int line) {
	object oheap = __object_from_interface(iallocator);
	dbg_assert(__cast(oheap, IALLOCATOR_ID) == iallocator);
...
#else 
inline void* iallocator_acquire_c(_interface iallocator, int size) {
	object oheap = __object_from_interface(iallocator);
	dbg_assert(__cast(oheap, IALLOCATOR_ID) == iallocator);

	return ((struct iallocator_vtable*)iallocator->__vtable)->__acquire(oheap, size);
}
inline bool iallocator_release_c(_interface iallocator, void* buff) {
	object oheap = __object_from_interface(iallocator);
	dbg_assert(__cast(oheap, IALLOCATOR_ID) == iallocator);
...
#endif
inline object iallocator_get_parent(_interface iallocator) {
	object oheap = __object_from_interface(iallocator);
	dbg_assert(__cast(oheap, IALLOCATOR_ID) == iallocator);
...
inline void iallocator_walk(_interface iallocator, pf_process_block per_block_cb, void* param) {
	object oheap = __object_from_interface(iallocator);
	dbg_assert(__cast(oheap, IALLOCATOR_ID) == iallocator);
