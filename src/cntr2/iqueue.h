...
extern inline       void     iqueue_destroy     (_interface iq);
extern inline       void     iqueue_clear       (_interface iq);
extern inline       void     iqueue_foreach     (_interface iq, pf_ref_process_v process, void* context);
extern inline       int      iqueue_size        (const_interface iq);
extern inline       bool     iqueue_empty       (const_interface iq);
extern inline const_unknown  iqueue_front       (const_interface iq);
extern inline const_unknown  iqueue_back        (const_interface iq);
extern inline       void     iqueue_push        (_interface iq, const_unknown __ref);
extern inline       void     iqueue_pop         (_interface iq);
extern inline       iterator iqueue_itr_create  (const_interface iq, itr_pos pos);
extern inline       void     iqueue_itr_assign  (const_interface iq, iterator itr, itr_pos pos);
extern inline const_iterator iqueue_itr_begin   (const_interface iq);
extern inline const_iterator iqueue_itr_end     (const_interface iq);
