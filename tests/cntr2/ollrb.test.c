...
static void __as_set_basic_test() {
	iset set = cntr_create_ollrb(int_compare);
	set_test_basic(set);
	iobject_destroy(set);
}

static void __as_mset_basic_test() {
	imset mset = cntr_create_ollrb(int_compare);
	mset_test_basic(mset);
	iobject_destroy(mset);
}

static void __as_set_bench_test() {
	iset set = cntr_create_ollrb_a(int_compare, NULL);
	set_test_bench(set);
	iobject_destroy(set);
}

static void __as_mset_bench_test() {
	imset mset = cntr_create_ollrb_a(int_compare, NULL);
